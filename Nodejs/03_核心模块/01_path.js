...
//     }
// )
const nfs = require("node:fs/promises")
//
// console.log(nfs.readFileSync('./hello.txt').toString());

// nfs.readFile(path.resolve(__dirname,'./hello.txt'),(err,data)=>{
//     if (err){
//         console.log('读取错误');
//     }else {
...
nfs.readFile(path.resolve(__dirname,"./hello.txt"))
    .then(data=>{
        console.log(data.toString());
    })
    .catch(e=>{
...
(async ()=>{
   try {
       const buffer = await nfs.readFile(path.resolve(__dirname,"./hello.txt"));
       console.log(buffer.toString());
   } catch (e){
       console.log('出现错误',e);
   }
